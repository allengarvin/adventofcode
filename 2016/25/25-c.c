...
int run_program(struct operation *prog, int plen, int reg[], char lines[][20]) {
    int pc = 0, cnt = 0, prev = 1;
...
int main(int argc, char *argv[]) {
    ...
    int prog_length, registers[4] = { 0, 0, 0, 0 }, i;
    char buf[20];
    FILE *fd;

    fd = fopen("25-input.txt", "r");
    for( prog_length = 0; fgets(buf, 19, fd); prog_length++ ) {
        ...
        switch( opcode[0] ) {
            case 'c': 
                ...
                break; 
            case 'o':
                program[prog_length].opcode = OUT; 
    ...
    for( i = 1; ; i++ ) {
        /* Properly I should be checking if the register state repeats 
           but this gets the right answer and it's quick */
        registers[0] = i;
        registers[1] = registers[2] = registers[3] = 0;
        if( run_program(program, prog_length, registers, program_contents) )
            break;
    }
    printf("%d\n", i);
}
