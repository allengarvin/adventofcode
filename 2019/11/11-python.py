    ...
    def read_instruction(self):
        ...
        if self.pc in self.instructions:
            ...
            if op.byte_list == op_byte_list:
                ...
                self.debug("INTEREST: opcode/arguments changed @{}".format(self.pc))
...
def run_robot(program_bytes: list, part2: bool):
    if part2:
        map = { 0 : 1 }
    else:
        map = { }
    moves = { 0 : -1j, 1 : 1, 2 : 1j, 3 : -1 }
    ...
    while True:
        current = map.get(position, 0)
        
        if loop1:
            computer.run_program(input_queue=[current])
            loop1 = False
        else:
            computer.continue_run(input_queue=[current])
        if computer.halted:
            break
        map[position] = computer.output_queue.pop(0)
        painted[position] = painted.get(position, 0) | map[position]

        if computer.output_queue.pop(0):
            orientation = (orientation + 1) % 4
        else:
            orientation = (orientation - 1) % 4
        position += moves[orientation]
...
def main(args):
    ...
    map, painted = run_robot(program_bytes, False)
    print(len(painted))
    map, painted = run_robot(program_bytes, True)
    ...
    for y in range(ly, by+1):
        s = ""
        for x in range(lx, bx+1):
            s += "#" if map.get(x + y * 1j) else " "
        print(s)
